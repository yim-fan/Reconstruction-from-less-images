...
from model import get_model, get_loss, compute_space_carving_loss
...
def train_step(step, model, device, dataset, batch, loss_fn, args, DEPTH_SCALES, DEPTH_SHIFTS):
    ...
    img_i = np.random.choice(img_idx)
    ...
    target_hypothesis = dataset.get_depth_hypothesis(img_i)
    ...
    with torch.no_grad():
        ...
        feature_map, attn = model.evaluate(rayo, rayd, c2w, step=step)
    ...
    if args.mask_corners:
        ...
        space_carving_mask = torch.ones((tgt.shape[0], tgt.shape[1]), dtype=torch.float, device=tgt.device)
...
def train_and_eval(start_step, model, device, dataset, eval_dataset, losses, args):
    ...
    DEPTH_SCALES = torch.autograd.Variable(torch.ones((model.points.shape[0], 1), dtype=torch.float, device=model.device)*args.scale_init, requires_grad=True)
    DEPTH_SHIFTS = torch.autograd.Variable(torch.ones((model.points.shape[0], 1), dtype=torch.float, device=model.device)*args.shift_init, requires_grad=True)
    ...
    while step < args.training.steps:
        for _, batch in enumerate(trainloader):
            ...
            loss, out = train_step(step, model, device, dataset, batch, loss_fn, args, DEPTH_SCALES, DEPTH_SHIFTS)
            if step < args.freeze_ss:
                optimizer_ss.step()
            avg_train_loss += loss
            step += 1
            eval_step_cnt += 1
