...
def load_meta_data(args, mode="train"):
    ...
    if args.type == "synthetic":
        images, poses, hwf, image_paths, all_depth_hypothesis = load_blender_data(
            args.path, args.cimle_dir, split=mode, factor=args.factor, read_offline=args.read_offline)
    ...
    all_depth_hypothesis = torch.from_numpy(all_depth_hypothesis).float()

    return images, poses, H, W, focal_x, focal_y, image_paths, all_depth_hypothesis
